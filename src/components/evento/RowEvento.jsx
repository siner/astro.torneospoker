...
export default function RowEvento(props) {
  ...
  const backgroundColor = evento.casinos.color;


  return (
    <div class="rowcasino flex bg-slate-300 hover:bg-slate-400">
      <div className="p-2 w-2/12 md:w-1/12 flex items-center">
          <a href={"/evento/" + evento.slug}>
            <img className="mx-auto max-h-12" src={evento.logo} alt={evento.name} />
          </a>
      </div>
      <div className="w-8/12 md:w-10/12 flex flex-col gap-4 md:flex-row md:items-center p-5 py-2">
        <div className="flex justify-between text-xs md:text-base mr-10">
          <div className="flex content-center space-x-1">
            ...
            <span>{datestringfrom} - {datestringto}</span>
          </div>
        </div>
      ...
      <div
        ...
        style={{ backgroundColor: backgroundColor }}>
          <img className="mx-auto block max-h-6" src={evento.casinos.logo} alt={evento.casinos.name} />
      </div>
